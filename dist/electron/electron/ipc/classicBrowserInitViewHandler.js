...
exports.registerClassicBrowserInitViewHandler = registerClassicBrowserInitViewHandler;
const electron_1 = require("electron");
const ipcChannels_1 = require("../../shared/ipcChannels");
// Optional: Define a logger utility or use console
const logger = {
    debug: (...args) => console.log('[IPCClassicBrowserInit]', ...args),
    warn: (...args) => console.warn('[IPCClassicBrowserInit]', ...args),
    error: (...args) => console.error('[IPCClassicBrowserInit]', ...args),
};
function registerClassicBrowserInitViewHandler(classicBrowserService) {
    electron_1.ipcMain.handle(ipcChannels_1.CLASSIC_BROWSER_INIT_VIEW, async (_event, { windowId, bounds, initialUrl }) => {
        logger.debug(`Handling ${ipcChannels_1.CLASSIC_BROWSER_INIT_VIEW} for windowId: ${windowId} with bounds: ${JSON.stringify(bounds)}, initialUrl: ${initialUrl}`);
...
//# sourceMappingURL=classicBrowserInitViewHandler.js.map