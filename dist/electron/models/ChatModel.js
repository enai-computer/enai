...
class ChatModel {
    ...
    async createSession() {
        ...
        logger_1.logger.debug(`[ChatModel] Creating new chat session with ID: ${sessionId}`);
        try {
            ...
            if (!newSession) {
                logger_1.logger.error(`[ChatModel] Failed to retrieve newly created session ${sessionId}`);
                throw new Error('Failed to retrieve newly created session');
            }
            logger_1.logger.info(`[ChatModel] Chat session created successfully: ${sessionId}`);
            return newSession;
        }
        catch (error) {
            logger_1.logger.error(`[ChatModel] Error creating chat session:`, error);
