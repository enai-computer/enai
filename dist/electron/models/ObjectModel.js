...
class ObjectModel {
    ...
    `);
        try {
            ...
            logger_1.logger.debug(`[ObjectModel] Created object synchronously with ID: ${newId}`);
            ...
            if (error.code === 'SQLITE_CONSTRAINT_UNIQUE' && data.sourceUri) {
                logger_1.logger.error(`[ObjectModel] Unique constraint violation in createSync for source_uri: ${data.sourceUri}`);
                throw new Error(`Object with source_uri '${data.sourceUri}' already exists`);
            }
            logger_1.logger.error(`[ObjectModel] Failed to create object synchronously:`, error);
            throw error;
        }
    }
    /**
    ...
    */
     ...
     */
    async countObjectsByStatus(status) {
        const db = this.db;
    ...
    `);
        try {
            const result = stmt.get(...statuses);
            logger_1.logger.debug(`[ObjectModel] Counted ${result.count} objects with status(es): ${statuses.join(', ')}`);
            return result.count;
        }
        catch (error) {
