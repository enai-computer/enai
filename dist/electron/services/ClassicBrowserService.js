...
const logger = {
    debug: (...args) => console.log('[ClassicBrowserService]', ...args),
    warn: (...args) => console.warn('[ClassicBrowserService]', ...args),
    error: (...args) => console.error('[ClassicBrowserService]', ...args),
};
class ClassicBrowserService {
    ...
    createBrowserView(windowId, bounds, initialUrl) {
        ...
        const view = new electron_1.BrowserView({ webPreferences: securePrefs });
        ...
        switch (action) {
            case 'back':
                if (wc.canGoBack())
                    ...
                    logger.warn(`windowId ${windowId}: Cannot go back, no history.`);
                break;
            case 'forward':
                if (wc.canGoForward())
                    wc.goForward();
                else
                    logger.warn(`windowId ${windowId}: Cannot go forward, no history.`);
                break;
            case 'reload':
                wc.reload();
                break;
