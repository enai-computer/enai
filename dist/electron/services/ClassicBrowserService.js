...
class ClassicBrowserService {
    ...
    async isSignificantNavigation(windowId, newUrl) {
    ...
    createBrowserView(windowId, bounds, initialUrl) {
        logger_1.logger.debug(`Attempting to create WebContentsView for windowId: ${windowId}`);
        if (this.views.has(windowId)) {
            logger_1.logger.warn(`WebContentsView for windowId ${windowId} already exists. Loading new URL.`);
        ...
        logger_1.logger.debug('Electron version:', process.versions.electron);
        const securePrefs = {
        ...
        logger_1.logger.debug(`✅ setBorderRadius called for windowId: ${windowId} with radius: ${borderRadius}px`);
        logger_1.logger.debug('BorderRadius fn typeof:', typeof view.setBorderRadius);
        logger_1.logger.debug('proto chain contains setBorderRadius?', 'setBorderRadius' in Object.getPrototypeOf(view));
        // Temporarily set background color to transparent (Checklist Item 3.7)
        view.setBackgroundColor('#00000000');
        logger_1.logger.debug(`windowId ${windowId}: Set background color to transparent for testing.`);
        if (!this.mainWindow || this.mainWindow.isDestroyed()) {
            logger_1.logger.error('Main window is not available to attach WebContentsView.');
        ...
        wc.on('did-start-loading', () => {
            logger_1.logger.debug(`windowId ${windowId}: did-start-loading`);
            this.sendStateUpdate(windowId, { isLoading: true, error: null });
        });
        wc.on('did-stop-loading', () => {
            logger_1.logger.debug(`windowId ${windowId}: did-stop-loading`);
        ...
        wc.on('did-navigate', async (_event, url) => {
            logger_1.logger.debug(`windowId ${windowId}: did-navigate to ${url}`);
            this.sendStateUpdate(windowId, {
                currentUrl: url,
            ...
            try {
                ...
                logger_1.logger.error('[ClassicBrowserService] Failed to log navigation activity:', logError);
            }
        });
        wc.on('page-title-updated', (_event, title) => {
            logger_1.logger.debug(`windowId ${windowId}: page-title-updated to ${title}`);
            this.sendStateUpdate(windowId, { title });
        });
        wc.on('page-favicon-updated', (_event, favicons) => {
            logger_1.logger.debug(`windowId ${windowId}: page-favicon-updated with ${favicons.length} favicons`);
        ...
        wc.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL) => {
            logger_1.logger.error(`windowId ${windowId}: did-fail-load for ${validatedURL}. Code: ${errorCode}, Desc: ${errorDescription}`);
        ...
        wc.on('render-process-gone', (_event, details) => {
            logger_1.logger.error(`windowId ${windowId}: render-process-gone. Reason: ${details.reason}`);
            this.sendStateUpdate(windowId, {
                isLoading: false,
        ...
        wc.setWindowOpenHandler((details) => {
            logger_1.logger.debug(`windowId ${windowId}: Intercepted new window request to ${details.url}`);
        ...
        wc.on('will-navigate', (event, url) => {
            logger_1.logger.debug(`windowId ${windowId}: will-navigate to ${url}`);
        ...
        wc.on('did-attach-webview', (event, webContents) => {
            logger_1.logger.debug(`windowId ${windowId}: Attached webview, setting up handlers`);
            webContents.setWindowOpenHandler((details) => {
                logger_1.logger.debug(`windowId ${windowId}: Iframe intercepted new window request to ${details.url}`);
        ...
        wc.on('focus', () => {
            logger_1.logger.debug(`windowId ${windowId}: WebContentsView received focus.`);
            ...
            if (this.mainWindow && !this.mainWindow.isDestroyed() && view) {
                ...
                logger_1.logger.debug(`windowId ${windowId}: Native view brought to front via addChildView on focus.`);
            }
        ...
        if (initialUrl) {
            logger_1.logger.debug(`windowId ${windowId}: Loading initial URL: ${initialUrl}`);
            // wc.loadURL(initialUrl); // loadUrl method will handle this
            this.loadUrl(windowId, initialUrl).catch(err => {
                logger_1.logger.error(`windowId ${windowId}: Failed to load initial URL ${initialUrl}:`, err);
                // State update for error already handled by did-fail-load typically
            });
        }
        logger_1.logger.debug(`WebContentsView for windowId ${windowId} created and listeners attached.`);
    }
    async loadUrl(windowId, url) {
        const view = this.views.get(windowId);
        if (!view) {
            logger_1.logger.error(`loadUrl: No WebContentsView found for windowId ${windowId}`);
            throw new Error(`WebContentsView with ID ${windowId} not found.`);
        }
        if (!url || typeof url !== 'string') {
            logger_1.logger.error(`loadUrl: Invalid URL provided for windowId ${windowId}: ${url}`);
            throw new Error('Invalid URL provided.');
        }
        logger_1.logger.debug(`windowId ${windowId}: Loading URL: ${url}`);
        ...
        try {
            ...
            logger_1.logger.error(`windowId ${windowId}: Error loading URL ${url}:`, error);
            this.sendStateUpdate(windowId, {
                isLoading: false,
    ...
    navigate(windowId, action) {
        const view = this.views.get(windowId);
        if (!view) {
            logger_1.logger.error(`navigate: No WebContentsView found for windowId ${windowId}`);
            // Optionally throw new Error(`WebContentsView with ID ${windowId} not found.`);
            return; // Or throw, depending on desired strictness
        }
        logger_1.logger.debug(`windowId ${windowId}: Performing navigation action: ${action}`);
        const wc = view.webContents;
        switch (action) {
            case 'back':
                if (wc.canGoBack())
                    ...
                    logger_1.logger.warn(`windowId ${windowId}: Cannot go back, no history.`);
                break;
            case 'forward':
                if (wc.canGoForward())
                    wc.goForward();
                else
                    logger_1.logger.warn(`windowId ${windowId}: Cannot go forward, no history.`);
                break;
            case 'reload':
                wc.reload();
                break;
            case 'stop':
                ...
                break;
            default:
                logger_1.logger.warn(`windowId ${windowId}: Unknown navigation action: ${action}`);
    ...
    setBounds(windowId, bounds) {
        const view = this.views.get(windowId);
        if (!view) {
            logger_1.logger.warn(`setBounds: No WebContentsView found for windowId ${windowId}. Cannot set bounds.`);
            return;
        }
        logger_1.logger.debug(`windowId ${windowId}: Setting bounds to ${JSON.stringify(bounds)}`);
        view.setBounds(bounds);
    }
    setVisibility(windowId, shouldBeDrawn, isFocused) {
        const view = this.views.get(windowId);
        if (!view) {
            logger_1.logger.warn(`setVisibility: No WebContentsView found for windowId ${windowId}. Cannot set visibility.`);
            return;
        }
        if (!this.mainWindow || this.mainWindow.isDestroyed()) {
            logger_1.logger.error('setVisibility: Main window is not available.');
            return;
        }
        ...
        logger_1.logger.debug(`windowId ${windowId}: Setting visibility - shouldBeDrawn: ${shouldBeDrawn}. 'isFocused' (${isFocused}) is ignored for stacking here.`);
        const viewIsAttached = this.mainWindow.contentView.children.includes(view);
        if (shouldBeDrawn) {
            if (!viewIsAttached) {
                this.mainWindow.contentView.addChildView(view);
                logger_1.logger.debug(`windowId ${windowId}: Attached WebContentsView because shouldBeDrawn is true and not attached.`);
            }
            ...
            if (isFocused) {
                ...
                logger_1.logger.debug(`windowId ${windowId}: Explicitly brought to front in setVisibility (shouldBeDrawn=true, isFocused=true).`);
            }
        }
        else { // Not to be drawn (e.g., minimized or window explicitly hidden)
            view.setVisible(false); // Make it not drawable
            logger_1.logger.debug(`windowId ${windowId}: Set WebContentsView to not visible because shouldBeDrawn is false.`);
            if (viewIsAttached) {
                this.mainWindow.contentView.removeChildView(view);
                logger_1.logger.debug(`windowId ${windowId}: Removed WebContentsView from contentView because shouldBeDrawn is false.`);
            }
        }
    }
    async destroyBrowserView(windowId) {
        const view = this.views.get(windowId);
        if (!view) {
            logger_1.logger.warn(`destroyBrowserView: No WebContentsView found for windowId ${windowId}. Nothing to destroy.`);
            return;
        }
        logger_1.logger.debug(`windowId ${windowId}: Destroying WebContentsView.`);
            ...
            try {
                ...
                if (!wc.isLoading()) {
                    ...
                    `);
                        ...
                        logger_1.logger.debug(`windowId ${windowId}: Script execution error (ignored):`, scriptError);
                    }
                }
                // Small delay to ensure media cleanup takes effect
                await new Promise(resolve => setTimeout(resolve, 100));
                logger_1.logger.debug(`windowId ${windowId}: Stopped media playback and cleared page.`);
            }
            catch (error) {
                logger_1.logger.debug(`windowId ${windowId}: Error during media cleanup:`, error);
        ...
        logger_1.logger.debug(`windowId ${windowId}: WebContentsView destroyed and removed from map.`);
    }
    async destroyAllBrowserViews() {
        logger_1.logger.debug('Destroying all WebContentsViews.');
...
//# sourceMappingURL=ClassicBrowserService.js.map