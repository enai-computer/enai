...
class SliceService {
    ...
    async getDetailsForSlices(chunkIds) {
        ...
        logger_1.logger.debug(`[SliceService] Fetching details for ${chunkIds.length} chunk IDs: [${chunkIds.slice(0, 5).join(', ')}]...`);
        try {
            ...
            if (chunks.length === 0) {
                logger_1.logger.debug("[SliceService] No chunks found for the provided IDs.");
                return [];
            }
            ...
            if (objectIds.length === 0) {
                ...
                logger_1.logger.warn("[SliceService] Chunks found but no associated object IDs extracted.");
            ...
            logger_1.logger.debug(`[SliceService] Fetched source metadata for ${sourceMetadataMap.size} unique object IDs.`);
            ...
            logger_1.logger.error(`[SliceService] Error fetching details for chunk IDs [${chunkIds.slice(0, 5).join(', ')}]...:`, error);
