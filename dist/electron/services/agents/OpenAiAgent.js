...
const chunkSchema = zod_1.z.array(zod_1.z.object({
    ...
    tags: zod_1.z.array(zod_1.z.string()).min(3).max(7),
    propositions: zod_1.z.array(zod_1.z.string()).min(2),
}));
/** Zod validator for object-level summary responses */
const objectSummarySchema = zod_1.z.object({
...
class OpenAiAgent {
     ...
     */
    async generateObjectSummary(cleanedText, title, objectId) {
        ...
        const initialMessages = [
            new messages_1.SystemMessage("You MUST only reply with valid JSON matching the requested schema. Do NOT include markdown code blocks."),
            new messages_1.HumanMessage(userPrompt),
        ];
        // Token counting
        const inputTokens = this.countTokens(initialMessages.map(m => m.content).join('\n'));
        logger_1.logger.debug(`[OpenAiAgent] Object ${objectId}: Generating object summary. Input tokens: ~${inputTokens}`);
        let attempt = 1;
        let lastError = null;
        while (attempt <= 2) { // Max 2 attempts (initial + 1 retry)
            try {
                logger_1.logger.info(`[OpenAiAgent] Object ${objectId}: Object summary attempt ${attempt}...`);
                const response = await this.llmService.generateChatResponse(initialMessages, {
                    userId: 'system',
                ...
                try {
                    ...
                    logger_1.logger.info(`[OpenAiAgent] Object ${objectId}: Successfully generated object summary.`);
                    ...
                    if (attempt === 1) {
                        ...
                        // Validation failed on the retry attempt
                        logger_1.logger.error(`[OpenAiAgent] Object ${objectId}: Object summary failed on retry validation: ${validationError.message}`);
                        throw validationError; // Throw the final validation error
                    }
                }
            }
                ...
                lastError = apiError;
                logger_1.logger.error(`[OpenAiAgent] Object ${objectId}: Object summary attempt ${attempt} failed API call: ${apiError.message}`);
                if (attempt === 1) {
                    logger_1.logger.info(`[OpenAiAgent] Object ${objectId}: Retrying API call after delay...`);
                    await this.wait(RETRY_DELAY_MS); // Wait before retry
                }
        ...
        // If loop finishes without returning/throwing, it means the second attempt failed
        logger_1.logger.error(`[OpenAiAgent] Object ${objectId}: Object summary generation failed after ${attempt - 1} attempts.`);
     ...
     * @throws Error if JSON parsing or Zod validation fails.
     */
    parseAndValidateObjectSummary(rawJsonString, objectId) {
        let jsonData;
        try {
            // Clean potential markdown code fences or leading/trailing whitespace
            const cleanedJsonString = rawJsonString.trim().replace(/^```json\s*|\s*```$/g, '');
            ...
            logger_1.logger.error(`[OpenAiAgent] Object ${objectId}: Failed to parse LLM response as JSON for object summary. Content: "${rawJsonString.substring(0, 100)}..."`, parseError);
            throw new Error(`Failed to parse LLM response as JSON: ${parseError.message}`);
        }
        const validationResult = objectSummarySchema.safeParse(jsonData);
        if (!validationResult.success) {
            logger_1.logger.error(`[OpenAiAgent] Object ${objectId}: LLM object summary response failed Zod validation. Errors: ${JSON.stringify(validationResult.error.flatten())}`);
