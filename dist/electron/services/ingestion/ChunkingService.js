...
const OpenAiAgent_1 = require("../../services/agents/OpenAiAgent");
...
class ChunkingService {
     ...
     */
    async processPdfObject(obj) {
        const objectId = obj.id;
        try {
            logger_1.logger.debug(`[ChunkingService] Processing PDF object ${objectId}: ${obj.title}`);
            ...
            if (existingChunks.length > 1) {
                logger_1.logger.warn(`[ChunkingService] PDF object ${objectId} has ${existingChunks.length} chunks, expected 1. Processing only the first.`);
            }
            const pdfChunk = existingChunks[0];
            logger_1.logger.debug(`[ChunkingService] Found PDF chunk ID ${pdfChunk.id} for object ${objectId}`);
            ...
            logger_1.logger.debug(`[ChunkingService] PDF object ${objectId}: Creating embedding...`);
            ...
            logger_1.logger.info(`[ChunkingService] PDF object ${objectId}: Successfully created embedding with vector ID ${vectorId}`);
            ...
            try {
                ...
                logger_1.logger.debug(`[ChunkingService] PDF object ${objectId}: Successfully linked embedding record`);
            }
            catch (linkError) {
                logger_1.logger.error(`[ChunkingService] PDF object ${objectId}: Failed to store embedding link:`, linkError);
