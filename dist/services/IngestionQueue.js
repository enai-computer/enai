    ...
    try {
        ...
        if (!parsedContent) {
            ...
            await objectModel.update(job.objectId, Object.assign(Object.assign({ status: 'error', errorInfo: 'Parsing failed or no content found after fetch.' }, (fetchResult.finalUrl !== job.sourceUri && { sourceUri: fetchResult.finalUrl })), { 
                ...
                parsedContentJson: null, cleanedText: null, parsedAt: undefined }));
        }
        else {
            ...
            await objectModel.update(job.objectId, Object.assign({ status: 'parsed', title: parsedContent.title, parsedContentJson: JSON.stringify(parsedContent), cleanedText: cleanedText, parsedAt: new Date(), errorInfo: null }, (fetchResult.finalUrl !== job.sourceUri && { sourceUri: fetchResult.finalUrl })));
        }
    }
    catch (error) {
...
async function queueForContentIngestion(objectId, sourceUri, objectModel) {
    ...
    await queue.add(async () => {
        ...
        try {
            await objectModel.update(objectId, { status: 'error', errorInfo: `Failed during queue operation: ${(_a = error === null || error === void 0 ? void 0 : error.message) !== null && _a !== void 0 ? _a : error}` });
        }
        catch (updateError) {
