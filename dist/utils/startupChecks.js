...
exports.checkChunkIdConsistency = checkChunkIdConsistency;
const logger_1 = require("./logger"); // Assuming logger path
 ...
 */
async function checkChunkIdConsistency(chromaClient, db, embeddingFunction // Added parameter
) {
    var _a;
    logger_1.logger.info('[Startup Check] Performing Chunk ID consistency check...');
    let sampleChunkIds = [];
    try {
        ...
        const rows = db.prepare(`SELECT chunk_id FROM chunks LIMIT ${SAMPLE_SIZE}`).all();
        sampleChunkIds = rows.map(row => row.chunk_id);
        if (sampleChunkIds.length === 0) {
            logger_1.logger.info('[Startup Check] No chunks found in SQLite, skipping consistency check.');
            return; // Nothing to check if DB is empty
        }
        logger_1.logger.debug(`[Startup Check] Sample SQLite chunk IDs: [${sampleChunkIds.join(', ')}]`);
        ...
        try {
            ...
            if ((_a = collectionError.message) === null || _a === void 0 ? void 0 : _a.includes('Could not find collection')) {
                logger_1.logger.warn(`[Startup Check] Chroma collection '${CHROMA_COLLECTION_NAME}' not found. Skipping check (likely first run or empty collection).`);
        ...
        if (missingIds.length > 0) {
            logger_1.logger.error(`[Startup Check] FAILED: Found ${missingIds.length} chunk ID(s) in SQLite that are MISSING in Chroma collection '${CHROMA_COLLECTION_NAME}'!`);
            logger_1.logger.error(`[Startup Check] Missing IDs: [${missingIds.join(', ')}]`);
            logger_1.logger.error('[Startup Check] This indicates a potential problem with the ingestion pipeline or data corruption.');
            throw new Error('Chunk ID consistency check failed. Mismatch between SQLite and ChromaDB.');
        }
        logger_1.logger.info(`[Startup Check] PASSED: Verified ${sampleChunkIds.length} chunk IDs exist in both SQLite and Chroma collection '${CHROMA_COLLECTION_NAME}'.`);
    }
    catch (error) {
        // Catch errors from SQLite query or the rethrown Chroma errors
        logger_1.logger.error(`[Startup Check] Error during consistency check: ${error.message}`);
