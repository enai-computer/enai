 ...
 */
export function insertIfNew(url: string, urlHash: string): { id: number | null; wasNew: boolean } {
  try {
    ...
    if (!wasNew) {
      ...
      const row = stmtSelectId.get(urlHash) as { bookmark_id: number } | undefined;
      return { id: row?.bookmark_id ?? null, wasNew: false };
    } else {
      // If it *was* new, the insertInfo.lastInsertRowid gives us the ID
      return { id: Number(insertInfo.lastInsertRowid), wasNew: true };
    }
  } catch (error) {
    logger.error(`[BookmarkModel] Failed to insert/select bookmark for hash ${urlHash}:`, error);
 ...
 */
export function getExistingUrlHashes(): Set<string> {
  try {
    ...
    const rows = stmtSelectAllHashes.all() as { url_hash: string }[];
    return new Set<string>(rows.map((r) => r.url_hash));
  } catch (error) {
    logger.error('[BookmarkModel] Failed to retrieve existing URL hashes:', error);
