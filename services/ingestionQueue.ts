...
interface IngestionJob {
  objectId: string;    // Changed from bookmarkId
  sourceUri: string;   // Changed from url
  // attempt: number; // Attempt logic might be simplified or handled differently
}

// Keep track of URIs currently being processed to avoid duplicates
const processingUris = new Set<string>(); // Renamed from processingUrls
...
async function processJob(job: IngestionJob, objectModel: ObjectModel): Promise<void> {
  ...
  if (processingUris.has(job.sourceUri)) {
      logger.warn(`[IngestionQueue] URI ${job.sourceUri} is already being processed. Skipping duplicate job for object ${job.objectId}.`);
      return;
  }
  ...
  try {
    ...
    const parsedContent: ReadabilityParsed | null = parseHtml(fetchResult.html, fetchResult.finalUrl);

    // Check handles null or empty content scenario
    if (!parsedContent) {
      // Parsing failed or no content found
      logger.warn(`[IngestionQueue] Parsing failed or no content for object ${job.objectId}, URL: ${fetchResult.finalUrl}`);
      ...
      logger.info(`[IngestionQueue] Parsing successful for object ${job.objectId}, Title: ${parsedContent.title}`);
...
export async function queueForContentIngestion(objectId: string, sourceUri: string, objectModel: ObjectModel): Promise<void> {
  ...
  if (processingUris.has(sourceUri)) {
    logger.debug(`[IngestionQueue] URI ${sourceUri} is currently being processed. Not adding duplicate for object ${objectId}.`);
    return;
  }
  ...
  await queue.add(async () => {
    ...
    logger.error(`[IngestionQueue] Failed to add job for object ${objectId}, URI: ${sourceUri} to the queue:`, error);
