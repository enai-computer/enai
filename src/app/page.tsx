...
export default function WelcomePage() {
  ...
  const { streamingMessage, activeStreamId } = useIntentStream({
    onStart: () => {
    ...
    onEnd: (final, data) => {
      ...
      if (final) {
        setChatMessages(prev => [...prev, { id: data.messageId || `assistant-stream-${Date.now()}`, role: 'assistant', content: final, createdAt: new Date() }]);
      }
      setIsThinking(false);
    ...
    onError: (error) => {
      ...
      setChatMessages(prev => [...prev, { id: `error-stream-${Date.now()}`, role: 'assistant', content: `Sorry, an error occurred: ${error}`, createdAt: new Date() }]);
      setIsThinking(false);
      setShowPlaceholder(true);
