...
export function HumanComputerIcon({ onClick, isActive = false }: HumanComputerIconProps) {
  return (
    <button
      onClick={onClick}
      className={`w-8 h-8 ${isActive ? 'bg-birkin' : 'bg-step-3'} rounded-full flex items-center justify-center z-50 group hover:bg-birkin transition-all duration-200 cursor-pointer`}
    >
      <svg 
        width="16" 
        height="16" 
