...
/**
 * Specialized hook for managing BrowserView lifecycle
 * 
 * @param windowId - The window ID for the BrowserView
 * @param initialBounds - Initial bounds for the BrowserView
 * @param initialUrl - Initial URL to load
 * @param enabled - Whether the BrowserView should be created (default: true)
 */
export function useBrowserView(
  ...
  return useNativeResource(
    // Mount callback
    async () => {
      if (!enabled || !window.api?.classicBrowserCreate) {
        return;
      }
      
      const result = await window.api.classicBrowserCreate(windowId, initialBounds, initialUrl);
      if (!result?.success) {
    ...
    // Dependencies
    [windowId, enabled], // Don't include bounds/URL to prevent recreation on every change
    // Options
    {
      unmountDelay: 50,
      debug: true,
      debugLabel: `BrowserView[${windowId}]`
    }
  );
}